...
<required_reading>
Read `/home/guang/Projects/Experiments/image-comparation/.cursor/gsd-core/references/ai-evals.md` before auditing. This is your scoring framework.
</required_reading>
...
**Project skills:** check `.cursor/skills/` or `.agents/skills/`. **agent_skills:** self-load per @/home/guang/Projects/Experiments/image-comparation/.cursor/gsd-core/references/agent-skills-bootstrap.md — list skill subdirectories, read each `SKILL.md` (lightweight index ~130 lines), load specific `rules/*.md` as needed. Do NOT load full `AGENTS.md` files (100KB+ context cost). Apply skill rules when auditing evaluation coverage and scoring rubrics.
...
**ALWAYS use the Write tool** — never `Shell(cat << 'EOF')` or heredoc for file creation.
