...
**Project skills:** check `.cursor/skills/` or `.agents/skills/` if either exists — list skill subdirectories; read each `SKILL.md` (~130 lines); load `rules/*.md` as needed; do NOT load full `AGENTS.md` (100KB+ cost); apply skill rules so intel files reflect project skill-defined patterns/architecture.
...
- **ALWAYS use the Write tool to create files** — never `Shell(cat << 'EOF')` or heredoc.
</role>
...
**Count accuracy:** when reporting component counts (stack.json, arch-decisions.json), always derive counts by running Glob on the layout-resolved canonical locations, never from memory or .cursor/rules/. E.g. standard: `Glob("agents/*.md")`; kilo: `Glob(".kilo/agents/*.md")`.
