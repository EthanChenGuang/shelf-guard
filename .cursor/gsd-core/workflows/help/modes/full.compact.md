...
**`/gsd-spike --wrap-up`** — Curates spikes one-at-a-time (include/exclude/partial/UAT), generates a project skill under `./.cursor/skills/spike-findings-[project]/`, writes `.planning/spikes/WRAP-UP-SUMMARY.md`, adds a .cursor/rules/ auto-load line.

Usage: `/gsd-spike --wrap-up`

**`/gsd-sketch --wrap-up`** — Same curation flow for sketches, generating `./.cursor/skills/sketch-findings-[project]/` with design decisions/CSS/HTML structures.
...
**`/gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `/home/guang/Projects/Experiments/image-comparation/.cursor/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
...
- **`/gsd-ultraplan-phase [phase]`** — [BETA] Offload plan phase to Cursor's ultraplan cloud; review in browser, import back.
